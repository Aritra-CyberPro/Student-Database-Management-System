...
def login():
    if usernameEntry.get()=='' or passwordEntry.get()=='':
        ...
        messagebox.showinfo('Success','Welcome to SDMS')
        window.destroy()
        import sdms
    
    else:
        messagebox.showerror('Warning','Please enter correct login id and password')

window=Tk()

window.geometry('1280x700+0+0')
window.title('Login Panel of Student Database')

window.resizable(False,False)

backgroundImage=ImageTk.PhotoImage(file='Admin panel bg.jpg')

bgLablel=Label(window,image=backgroundImage)
bgLablel.place(x=0,y=0)

loginFrame=Frame(window,bg='white')
loginFrame.place(x=400,y=150) 

logoImage=PhotoImage(file='graduate_logo.png')
